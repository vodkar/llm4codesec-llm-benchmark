...
class VulDetectBenchDatasetLoader:
            ...
            with open(data_path, 'r', encoding='utf-8') as f:
                for i, line in enumerate(f):
                    ...
                    try:
                        ...
                        vuldetectbench_samples = self._convert_vuldetectbench_item_to_samples(
                            item, i, task_type
                        )
    ...
    def _convert_vuldetectbench_item_to_samples(
        ...
        if task_type == "task1":
            ...
            # Add selection choices to metadata
            selections = item.get("selection", "")
    ...
    def _get_vulnerability_severity(self, cwe: str) -> Optional[str]:
        ...
        high_severity_cwes = {
            "78", "79", "89", "94", "119", "120", "121", "122", "123", "124",
            "787", "788", "416", "415", "400"
        }
        medium_severity_cwes = {
            "125", "190", "476", "401", "403", "404", "502", "611", "776"
        }
    ...
    def create_dataset_from_vuldetectbench_data(
        ...
        Args:
            ...
            task_type: Type of task (task1, task2, task3, task4, task5)
        """
        self.logger.info(f"Creating dataset for {task_type} from {vuldetectbench_data_dir}")
        
        data_dir = Path(vuldetectbench_data_dir)
        if not data_dir.exists():
            raise FileNotFoundError(f"VulDetectBench data directory not found: {vuldetectbench_data_dir}")
        ...
        try:
            ...
            if data_path.suffix == '.json':
                with open(data_file, 'r', encoding='utf-8') as f:
                    ...
                    if isinstance(data, dict) and "samples" in data:
                        items = data["samples"]
                        stats["task_type"] = data.get("metadata", {}).get("task_type", "unknown")
                    else:
                        items = data
            else:  # JSONL format
                items = []
...
class VulDetectBenchDatasetLoaderFramework:
    ...
    def load_dataset(
        ...
        **kwargs
        ...
        return self.loader.load_dataset(
            dataset_path,
            task_type=vuldetectbench_task_type,
    ...
    def load_processed_dataset(
        ...
        Returns:
            list of BenchmarkSample objects
        """
        return self.loader.load_processed_dataset(
            dataset_path
        )
