...
class CVEFixesBenchmarkRunner:
    ...
    def run_benchmark(self, sample_limit: Optional[int] = None):
        ...
        try:
            ...
            # Print summary
            print(f"\n{'='*60}")
            print(f"CVEFixes Benchmark Results Summary")
            print(f"{'='*60}")
...
Examples:
  ...
  python run_cvefixes_benchmark.py --dataset-path datasets_processed/cvefixes/cvefixes_binary_c.json

  # Run CWE-specific detection
  python run_cvefixes_benchmark.py --task-type cwe_specific --cwe-type CWE-119 --dataset-path datasets_processed/cvefixes/cvefixes_cwe_119.json

  # Use custom model
  python run_cvefixes_benchmark.py --model-type custom --model-name "microsoft/codebert-base" --dataset-path datasets_processed/cvefixes/cvefixes_binary_c.json

  # Limit samples for testing
  python run_cvefixes_benchmark.py --dataset-path datasets_processed/cvefixes/cvefixes_binary_c.json --sample-limit 100
        """
    )
    ...
    parser.add_argument(
        "--system-prompt",
        type=str,
        help="Custom system prompt template"
    )
    ...
    try:
        ...
        if not args.model_name:
            ...
            config.model_name = model_names.get(args.model_type, "Qwen/Qwen2.5-7B-Instruct")
