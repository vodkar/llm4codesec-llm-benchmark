...
from datasets.loaders.vuldetectbench_dataset_loader import VulDetectBenchDatasetLoaderFramework
...
class VulDetectBenchBenchmarkRunner:
    ...
    def run_benchmark(self, sample_limit: int | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            logging.info(f"Loading VulDetectBench dataset from: {self.config.dataset_path}")
            samples = self.dataset_loader.load_processed_dataset(Path(self.config.dataset_path))
            ...
            for i, sample in enumerate(samples):
                ...
                if sample.metadata.get("task_type") == "task2" and "selection_choices" in sample.metadata:
                    # For Task 2, include the selection choices in the prompt
                    user_prompt = f"{sample.metadata['selection_choices']}\n{user_prompt}"
                ...
                prediction = PredictionResult(
                    ...
                    true_label=sample.label if isinstance(sample.label, int) else self._parse_task_specific_response(str(sample.label), sample.metadata.get("task_type", "task1")),
                    confidence=None,
            ...
            task_type = samples[0].metadata.get("task_type", "task1") if samples else "task1"
    ...
    def _calculate_task_specific_metrics(self, predictions, task_type: str) -> dict[str, Any]:
    ...
    def _calculate_code_analysis_metrics(self, predictions, task_type: str) -> dict[str, Any]:
        ...
        if task_type == "task3":
            ...
            for pred in predictions:
                recall = self._calculate_token_recall(pred.predicted_label, pred.true_label)
                token_recalls.append(recall)
            
            metrics["macro_token_recall"] = sum(token_recalls) / len(token_recalls) if token_recalls else 0.0
            ...
            for pred in predictions:
                line_recall = self._calculate_line_recall(pred.predicted_label, pred.true_label)
                union_line_recall = self._calculate_union_line_recall(pred.predicted_label, pred.true_label)
                
                line_recalls.append(line_recall)
                union_line_recalls.append(union_line_recall)
            
            metrics["avg_line_recall"] = sum(line_recalls) / len(line_recalls) if line_recalls else 0.0
            metrics["avg_union_line_recall"] = sum(union_line_recalls) / len(union_line_recalls) if union_line_recalls else 0.0
    ...
    try:
        ...
        if args.plan:
            ...
            results = run_single_experiment(
                args.model, args.dataset, args.prompt, config, 
                args.sample_limit, args.output_dir
            )
