...
class JitVulBatchRunner:
    ...
    def run_single_experiment(
        self, experiment_name: str, experiment_config: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            result_file = os.path.join(
                exp_output_dir, f"{experiment_name}_summary.json"
            )
            with open(result_file, "w") as f:
                json.dump(result_summary, f, indent=2, default=str)

            self.logger.info(
                f"Completed experiment: {experiment_name} (duration: {duration:.2f}s)"
            )
    ...
    def run_experiment_list(self, experiment_names: List[str]) -> Dict[str, Any]:
        ...
        for exp_name in experiment_names:
            ...
            if exp_config is None:
                self.logger.error(
                    f"Configuration for experiment '{exp_name}' not found"
                )
                results[exp_name] = {
                    "status": "error",
    ...
    def generate_batch_summary(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        summary: Dict[str, Any] = {
            ...
            "failed_experiments": len(
                [r for r in results.values() if r.get("status") == "error"]
            ),
            "total_duration": sum([r.get("duration", 0) for r in results.values()]),
            "average_duration": sum([r.get("duration", 0) for r in results.values()])
            / len(results)
            if results
            else 0,
            "experiment_details": {},
        ...
        for exp_name, result in results.items():
            if result.get("status") == "success" and "results" in result:
                ...
                summary["experiment_details"][exp_name] = {
                    ...
                    "num_samples": exp_results.get("dataset_info", {}).get(
                        "total_samples", 0
                    ),
                    "task_type": result.get("config", {})
                    .get("dataset_config", {})
    ...
    def save_batch_results(
        self, results: Dict[str, Any], batch_name: Optional[str] = None
    ) -> str:
        """
...
Examples:
  ...
  python run_jitvul_batch.py --config benchmarks/JitVul/jitvul_experiments_config.json --batch jitvul_full_evaluation --output results/my_batch --log-level DEBUG
    ...
    parser.add_argument("--batch", type=str, help="Name of batch configuration to run")
    ...
    try:
        ...
        results_file = runner.run(
            batch_name=args.batch, experiment_names=args.experiments
        )
        ...
        logger.info(
            f"Summary: {summary['successful_experiments']}/{summary['total_experiments']} experiments successful"
        )
