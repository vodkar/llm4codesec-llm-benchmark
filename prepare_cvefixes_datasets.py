...
def prepare_binary_datasets(loader: CVEFixesDatasetLoader, 
                          output_dir: Path,
    ...
    file_output = output_dir / f"cvefixes_binary_{programming_language.lower()}_file.json"
    ...
    method_output = output_dir / f"cvefixes_binary_{programming_language.lower()}_method.json"
...
def prepare_multiclass_datasets(loader: CVEFixesDatasetLoader,
                               output_dir: Path,
                               programming_language: str = "C",
                               sample_limit: Optional[int] = None):
    ...
    file_output = output_dir / f"cvefixes_multiclass_{programming_language.lower()}_file.json"
    ...
    method_output = output_dir / f"cvefixes_multiclass_{programming_language.lower()}_method.json"
...
def prepare_cwe_specific_datasets(loader: CVEFixesDatasetLoader,
                                output_dir: Path,
                                programming_language: str = "C",
    ...
    if not cwe_types:
        ...
        cwe_types = [
            "CWE-119", "CWE-120", "CWE-125", "CWE-190", 
            "CWE-476", "CWE-787", "CWE-401", "CWE-416",
            "CWE-20", "CWE-79", "CWE-89", "CWE-22"
        ]
...
def analyze_database(loader: CVEFixesDatasetLoader, output_dir: Path):
    ...
    # Print summary
    print(f"\n{'='*60}")
    print(f"CVEFixes Database Statistics")
    print(f"{'='*60}")
...
Examples:
  ...
  python prepare_cvefixes_datasets.py --database-path benchmarks/CVEfixes/Data/CVEfixes.db --dataset-types binary --sample-limit 1000

  # Prepare datasets for multiple languages
  python prepare_cvefixes_datasets.py --database-path benchmarks/CVEfixes/Data/CVEfixes.db --languages C Java

  # Just analyze the database
  python prepare_cvefixes_datasets.py --database-path benchmarks/CVEfixes/Data/CVEfixes.db --analyze-only
        """
    )
